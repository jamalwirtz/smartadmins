...
import ExamLayouts from './pages/ExamLayouts'
import TimetableLayouts from './pages/TimetableLayouts'
...
import {
  ...
  HelpCircle, GraduationCap, Menu, X as XIcon, Palette, LayoutTemplate
} from 'lucide-react'
...
const PAGE_TITLES = {
  ...
  '/exam-layouts': { label: 'Exam Schedule Editor', icon: <LayoutTemplate size={15}/> },
  '/timetable-layouts': { label: 'Timetable Schedule Editor', icon: <LayoutTemplate size={15}/> },
  '/onboarding/curriculum': { label: 'Curriculum Setup', icon: <GraduationCap size={15}/> },
  ...
  return (
    <motion.aside className="sidebar"
      ...
      <nav className="sidebar-nav">
        ...
        <NavItem to="/exam-layouts" icon={<LayoutTemplate size={16}/>} label="Exam Schedule Editor" onClick={closeOnMobile}/>
        <NavItem to="/timetable-layouts" icon={<LayoutTemplate size={16}/>} label="Timetable Schedule Editor" onClick={closeOnMobile}/>
        ...
        <NavItem to="/onboarding/curriculum" icon={<GraduationCap size={16}/>} label="Curriculum Setup" onClick={closeOnMobile}/>
      </nav>
...
// Auto-triggered for any school that hasn't completed curriculum setup yet.
// Checked once per session (not on every navigation) and dismissible —
// dismissing marks onboarding as complete with no curriculum selected, same
// as clicking "Skip" inside the wizard itself, so it never nags twice.
function OnboardingBanner({ onDismiss }) {
  const navigate = useNavigate()
  return (
    <motion.div className="onboarding-banner"
      initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:'auto' }}
      exit={{ opacity:0, height:0 }} transition={{ duration:.25 }}>
      <div className="onboarding-banner-inner">
        <div className="onboarding-banner-icon"><GraduationCap size={18}/></div>
        <div className="onboarding-banner-text">
          <div className="onboarding-banner-title">Set up your curriculum</div>
          <div className="onboarding-banner-sub">
            Pick CAIE, UNEB, IB, AP, or CBC to pre-fill subjects with the right codes — takes under a minute.
          </div>
        </div>
        <div className="onboarding-banner-actions">
          <button className="btn btn-accent btn-sm" onClick={() => navigate('/onboarding/curriculum')}>
            Set Up Now
          </button>
...
function ProtectedLayout({ children }) {
  ...
  // Check curriculum-onboarding status once the user is loaded. Skipped on
  // the onboarding page itself so the banner never sits above the wizard
  // that's meant to resolve it.
  useEffect(() => {
    if (!user) return
    curriculumAPI.getActive()
      .then(r => setShowOnboardingBanner(!r.data.onboarding_completed))
      .catch(() => {})
  }, [user])
  ...
  const dismissBanner = async () => {
    ...
    try {
      await curriculumAPI.setActive({ education_system_id: null, onboarding_completed: true })
    } catch { /* non-critical — banner just won't reappear this session */ }
  }
  ...
  const onOnboardingPage = location.pathname === '/onboarding/curriculum'
  ...
  return (
    <div className={`app-shell${sidebarOpen ? ' sidebar-open' : ''}`}>
      ...
      <div className="app-main">
        ...
        <AnimatePresence>
          {showOnboardingBanner && !onOnboardingPage && (
          ...
          <ProtectedLayout>
            <AnimatePresence mode="wait">
              <Routes>
                ...
                <Route path="/exam-layouts" element={<motion.div key="exl"   {...pageVariants}><ExamLayouts/></motion.div>}/>
                <Route path="/timetable-layouts" element={<motion.div key="ttl" {...pageVariants}><TimetableLayouts/></motion.div>}/>
                <Route path="/onboarding/curriculum" element={<motion.div key="curr" {...pageVariants}><CurriculumOnboarding/></motion.div>}/>
