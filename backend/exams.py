...
from models import (
    ...
    Subject, ClassSection, Teacher, Room, User,
)
...
class SlotCreate(BaseModel):
    ...
    room_id:        Optional[str] = None   # FIX: was missing — dropdown-selected Room never persisted
...
def _sync_room_name(slot: ExamSlot, db: Session) -> None:
    """
    FIX: keep the plain-text `room` column in sync with the FK-based `room_id`
    selection. Exports (PDF/Excel) and older code read `slot.room` as a string,
    so picking a Room from the dropdown needs to also populate that string —
    otherwise exports would show "—" even though a room was actually assigned.
    """
    if slot.room_id:
...
def _format_slot(slot: ExamSlot) -> dict:
    return {
        ...
        "room":         slot.room,
        "room_id":      slot.room_id,   # FIX: was missing — frontend edit form had no way to know the current room
        "notes":        slot.notes,
...
def update_slot(
    ...
    # FIX: if a room was picked via room_id (dropdown), keep the plain `room`
    # text column in sync so PDF/Excel exports show it correctly.
...
def generate(
    ...
    Note: `include_supervisors` / `include_rooms` only control whether those
    columns appear in PDF/Excel exports (see /export/exam routes and School
    Settings). This endpoint does not auto-assign invigilators or rooms —
    that is still a manual step from the exam grid's edit (pencil) button.
    """
    _session_or_404(session_id, db)
